...
export interface FolderSortUpdate {
  id: string;
  sortIndex: number;
}

export const providerFoldersApi = {
  async getAll(appType: string): Promise<ProviderFolder[]> {
    return await invoke("get_provider_folders", { appType });
  },

  async create(name: string, appType: string): Promise<ProviderFolder> {
  ...
  async delete(id: string, appType: string): Promise<void> {
